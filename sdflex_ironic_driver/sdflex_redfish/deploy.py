...
class SDflexHeartbeatMixin(agent_base.HeartbeatMixin):
    ...
    def configure_local_boot(self, task, root_uuid=None,
        ...
        This method creates a boot entry and, this method sets the
        ...
        if result['command_status'] == 'FAILED':
            ...
            self.log_and_raise_deployment_error(task, msg)
        try:
            persistent = True
        ...
        LOG.info('Boot entry is create and node will boot from pre-provisioned '
                 'volume for node %s', node.uuid)
