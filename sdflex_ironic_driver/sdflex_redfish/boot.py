...
class SdflexRedfishVirtualMediaBoot(redfish_boot.RedfishVirtualMediaBoot):
    ...
    def _publish_image(self, image_file, object_name, nfs_server_location):
        ...
        :param nfs_server_location: Server location address where the
                                    image file have to be placed
        :return: a URL to download published file
        """
        public_dir = nfs_server_location
    ...
    def _unpublish_image(self, object_name, nfs_server_location):
        ...
        """
        if nfs_server_location:
            published_file = os.path.join(nfs_server_location, object_name)
    ...
    def _cleanup_iso_image(self, task):
        ...
        nfs_server_location = driver_info.get('remote_image_share_root')
        iso_object_name = self._get_iso_image_name(task.node)
        self._unpublish_image(iso_object_name, nfs_server_location)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            with tempfile.NamedTemporaryFile(
                ...
                image_url = self._publish_image(
                    boot_iso_tmp_file, iso_object_name, nfs_server_location)
    ...
    def prepare_ramdisk(self, task, ramdisk_params):
        ...
        url = "nfs://" + remote_image_server + iso_ref
    ...
    def prepare_instance(self, task):
        ...
        sdflex_common.eject_vmedia(task,
                                   sdflexutils_constants.VIRTUALMEDIA_DEVICE0)
        sdflex_common.insert_vmedia(task, iso_ref,
                                    sdflexutils_constants.VIRTUALMEDIA_DEVICE0)
    ...
    def validate(self, task):
        ...
        driver_info = task.node.driver_info
        missing = []
        if 'remote_image_server' not in driver_info:
            missing.append('remote_image_server')
        if 'remote_image_share_root' not in driver_info:
            missing.append('remote_image_share_root')
        if missing:
            raise ironic_exception.MissingParameterValue(_(
