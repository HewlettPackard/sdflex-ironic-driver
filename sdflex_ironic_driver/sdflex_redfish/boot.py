...
class SdflexRedfishDhcplessBoot(pxe.PXEBoot):
    ...
    def validate(self, task):
        ...
        invalid_parameter = []
        if not ipv4_address:
            invalid_parameter.append('ipv4_address')
        if not ipv4_gateway:
            invalid_parameter.append('ipv4_gateway')
        if not ipv4_subnet_mask:
            invalid_parameter.append('ipv4_subnet_mask')
        if invalid_parameter:
            raise ironic_exception.InvalidParameterValue(_(
                "%(invalid_parameter)s are invalid parameters in Network data")
