...
class SdflexRedfishVirtualMediaBoot(redfish_boot.RedfishVirtualMediaBoot):
    ...
    def _publish_image(self, image_file, object_name):
        ...
        """
#         public_dir = CONF.sdflex.nfs_root
        public_dir = "/home/ubuntu/nfsfolder"
        ...
        image_url = os.path.join(public_dir, object_name)
        return image_url
    ...
    def _unpublish_image(cls, object_name):
        ...
        """
        published_file = os.path.join("/home/ubuntu/nfsfolder", object_name)
    ...
    def prepare_ramdisk(self, task, ramdisk_params):
        ...
        if config_via_floppy:
            ...
            sdflex_common.eject_vmedia(task, "device1")
            sdflex_common.insert_vmedia(task, floppy_ref, "device1")
...
#                             '%(node)s', {'node': task.node.uuid})
        ...
        url = "nfs://10.229.136.103" + iso_ref

#         sdflex_common._eject_vmedia(task, sushy.VIRTUAL_MEDIA_CD)
#         sdflex_common._insert_vmedia(task, iso_ref, sushy.VIRTUAL_MEDIA_CD)
        sdflex_common.eject_vmedia(task, "device0")
        sdflex_common.insert_vmedia(task, url, "device0")
    ...
    def prepare_instance(self, task):
        ...
        sdflex_common.eject_vmedia(task, "device0")
        sdflex_common.insert_vmedia(task, iso_ref, "device0")
    ...
    def _prepare_floppy_image(self, task, params=None):
        ...
        """
        object_name = self._get_floppy_image_name(task.node)
        ...
        with tempfile.NamedTemporaryFile(
            ...
            image_url = self._publish_image(vfat_image_tmpfile, object_name)
