...
#         image_url = os.path.join(public_dir, object_name)
    ...
    def prepare_ramdisk(self, task, ramdisk_params):
        ...
        import pdb;pdb.set_trace()
        if node.driver_info.get('remote_image_share_type')=='nfs':
            image_share_root = node.driver_info.get('remote_image_share_root')
        else:
            image_share_root = node.driver_info.get('image_share_root')
        ...
        if remote_server_data['remote_image_share_type'] == 'nfs':
            url = remote_server_data['remote_image_share_type'] + "://" + remote_image_server + image_share_root + "/"  + iso_ref
        else:
