...
class SdflexRedfishDhcplessBoot(test_common.BaseSdflexTest):
    ...
    @mock.patch.object(sdflex_common, 'set_network_setting_dhcpless_boot',
                       spec_set=True, autospec=True)
    @mock.patch.object(redfish_image_utils, 'prepare_deploy_iso', spec_set=True,
                       autospec=True)
    @mock.patch.object(boot_mode_utils, 'sync_boot_mode',
                       spec_set=True, autospec=True)
    @mock.patch.object(manager_utils, 'node_set_boot_device',
    ...
    def _test_prepare_ramdisk_needs_node_prep(self, pxe_prepare_ramdisk_mock,
                                              ...
                                              sync_boot_mode_mock,
                                              set_network_setting_dhcpless_boot_mock,
                                              prov_state):
        self.node.provision_state = prov_state
        self.node.save()
        with task_manager.acquire(self.context, self.node.uuid,
    ...
    def test_validate(self, func_validate):
        with task_manager.acquire(self.context, self.node.uuid,
                                  shared=False) as task:
            network_data = {'links':[{'id': 'enp1s0','type': 'phy', 'ethernet_mac_address': '94:40:C9:D6:03:84', 'mtu': 1500}],
                            'networks': [{'id': 'provisioning IPv4', 'type': 'ipv4', 'link': 'enp1s0','ip_address': '10.229.136.128',
                                          'netmask': '255.255.248.0',
                                          'routes': [{'network': '10.229.136.0', 'netmask': '255.255.248.0', 'gateway': '10.229.136.1'},
                                                     {'network': '0.0.0.0', 'netmask': '0.0.0.0', 'gateway': '10.229.136.1'}],
                                          'network_id': ''}],
                            'services': [{'type': 'dns', 'address': '10.229.136.1'}]}
