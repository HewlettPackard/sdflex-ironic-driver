...
class SdflexRedfishVirtualMediaBootTestCase(test_common.BaseSdflexTest):
    ...
    def setUp(self):
        ...
        self.config(enabled_hardware_types=['sdflex-redfish'],
                    ...
                    enabled_boot_interfaces=['sdflex-vmedia'],
    ...
    def test__publish_image_local_link(self, mock_chmod, mock_mkdir,
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            mock_chmod.assert_called_once_with(expected_url, 0o777)
            mock_link.assert_called_once_with('file.iso', expected_url)
    ...
    def test__cleanup_iso_image(self, mock_unpublish):
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            task.node.driver_info['nfs_root'] = "/home/ubuntu/nfsfolder"
    ...
    @mock.patch.object(boot_mode_utils, 'get_boot_mode_for_deploy',
                       autospec=True)
    def test_validate(self, mock_get_boot_mode,
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            task.node.driver_info.update(
                {'deploy_kernel': 'kernel',
                 'deploy_ramdisk': 'ramdisk',
                 'bootloader': 'bootloader',
                 'nfs_root': '/home/ubuntu/nfsfolder/',
                 'nfs_server': '1.2.3.4'}
            )
    ...
    def test_prepare_ramdisk_with_params(self, mock_boot_mode_utils,
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            expected_url = "nfs://1.2.3.4/image-url"
    ...
    def test_prepare_instance_normal_boot(
        ...
        with task_manager.acquire(self.context, self.node.uuid,
            ...
            expected_url = "nfs://1.2.3.4/image-url"
            mock__prepare_boot_iso.return_value = expected_url
